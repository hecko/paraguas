...
if ((mysql_num_rows($raw)>=1) and ($s==0)) {
  ...
  $row = mysql_fetch_assoc($raw);
  //execute this if we already have this message and received message has problem status or is an unpaired message
  ...
  if (($s==1) OR ($s==2)) {
	...
	echo "This is an OK message without matching PROBLEM pair.\n";
    $sql = "INSERT INTO active (name,source,source_ip,severity,message,contact_group,status,first_time,last_time,last_problem_time,count,notes) VALUES ('$n','$x','$source_ip',$i,'$m','$g',$s,$t,$t,$t,1,'$a')";
	if (mysql_query($sql)) {
		echo "New event received.\n";
	} else {
		echo $sql."\n";
		echo mysql_error();
	}
